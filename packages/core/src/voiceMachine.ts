...
function nextMessageId(role: 'user' | 'agent' | 'system' = 'user'): string {
  messageCounter = (messageCounter + 1) % 1_000_000;
  const now = typeof Date !== 'undefined' ? Date.now() : 0;
  return `${role}_${now}_${messageCounter}`;
}
      ...
      states: {
        listening: {
          on: {
            ...
            USER_TEXT: {
              target: 'processing',
              actions: [{ type: 'addUserMessage', params: ({ event }) => ({ text: event.text }) }],
            },
          },
        },
        processing: {
