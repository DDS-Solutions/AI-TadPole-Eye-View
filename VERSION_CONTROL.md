...
## 6. Versioning — repo-level semver, 0.x until maturity

Single version for the product; internal `@gev/*` packages stay `0.x` workspace-private until/unless npm-published (then adopt Changesets — not before; see §7).

**Scheme:** `vMAJOR.MINOR.PATCH` — during 0.x, MINOR = intentional breakage/milestone, PATCH = fixes.

| Tag | Gate to earn it |
|---|---|
| `v0.0.x` | Scaffold era (now) |
| `v0.1.0` | **Phase 0 exit:** CI green on Actions, one live feed keyless, governance stubs live, serializer working |
| `v0.2.0` | **Phase 1:** server parity, SSRF/proxy suite fully ported + property-tested |
| `v0.3.x` | Phase 2 batches — one MINOR per layer group merged |
| `v0.4.0` | Voice agent + provider-agnostic endpoint |
| `v0.5.0` | Collaboration tiers (T1 debug bundles, T2 Yjs rooms) |
| `v1.0.0` | **Explicit criteria:** 13-layer parity · keyless boot default · governance ≥ M2 · SECURITY.md threat model final · licensing clean (no bundled NC data) · e2e suite stable-green for 2 consecutive weeks |

M-rung merges (Tadpole M1–M4) get MINOR bumps + a release-notes section each — they're the demo milestones.

## 7. Releases & artifacts

Automated by **release-please** (derives versions + changelog entirely from conventional commits — zero per-PR ceremony, which matters when most committers are agents):
...
- Release PR accumulates merged commits → you merge it → it cuts the tag + GitHub Release + `CHANGELOG.md`.
...
- **Cadence:** continuous (release-please PR merged whenever meaningful), plus a guaranteed cut at each demo milestone. Never demo off `main` untagged — "works on v0.3.1" beats "works on my machine."
