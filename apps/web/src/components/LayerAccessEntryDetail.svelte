...
<section
  class="entry-detail"
  aria-labelledby={`layer-access-title-${entry.id}`}
>
  <header class="detail-header">
    <div>
      ...
      <p class="source-line">
        {entry.source.name} · {entry.runtime.mode.toUpperCase()}
      </p>
    </div>
    <span
      class:warning={entry.effective_access !== "available"}
      class="status-pill"
    >
  ...
  <div class="gate-grid" aria-label="Independent access gates">
    ...
    <section class="gate-card">
      ...
      {#if entry.credential.masked_fingerprint}
        <code aria-label="Masked credential fingerprint"
          >{entry.credential.masked_fingerprint}</code
        >
    ...
    <section class="gate-card">
      ...
      <small
        >Budget remaining: ${entry.policy.budget_remaining_usd.toFixed(
          2,
        )}</small
      >
    </section>
  ...
  <section class="detail-section">
    ...
    <dl class="time-grid mono">
      <div>
        <dt>Observation</dt>
        <dd>{displayTime(entry.runtime.observation_at)}</dd>
      </div>
      <div>
        <dt>Retrieved</dt>
        <dd>{displayTime(entry.runtime.retrieved_at)}</dd>
      </div>
      <div>
        <dt>Cache origin</dt>
        <dd>{displayTime(entry.runtime.cache_origin_at)}</dd>
      </div>
      <div>
        <dt>Last success</dt>
        <dd>{displayTime(entry.runtime.last_success_at)}</dd>
      </div>
      <div>
        <dt>Last error</dt>
        <dd>{displayTime(entry.runtime.last_error_at)}</dd>
      </div>
      <div>
        <dt>Next poll</dt>
        <dd>{displayTime(entry.runtime.next_poll_at)}</dd>
      </div>
    </dl>
  </section>
  ...
  <section class="detail-section">
    ...
    {#each entry.products as product}
      <article class="product">
        ...
        <small
          >{product.transport.toUpperCase()} · {product.formats.join(
            ", ",
          )}</small
        >
  ...
  <section class="detail-section policy-copy">
    ...
    <dl>
      <div>
        <dt>Refresh</dt>
        <dd class="mono">{entry.policy.refresh_seconds}s</dd>
      </div>
      <div>
        <dt>Fresh cache</dt>
        <dd class="mono">{entry.policy.fresh_cache_seconds}s</dd>
      </div>
      <div>
        <dt>Maximum stale</dt>
        <dd class="mono">{entry.policy.max_stale_seconds}s</dd>
      </div>
      <div>
        <dt>Rate</dt>
        <dd>{entry.policy.upstream_rate_limit}</dd>
      </div>
      <div>
        <dt>Budget</dt>
        <dd>{entry.policy.budget_policy}</dd>
      </div>
      <div>
        <dt>Fallback</dt>
        <dd>{entry.policy.fallback}</dd>
      </div>
    </dl>
  </section>
  ...
  <nav class="source-links" aria-label="Authoritative source links">
    <a href={entry.credential.setup_url} target="_blank" rel="noreferrer"
      >Setup</a
    >
    <a href={entry.terms.terms_url} target="_blank" rel="noreferrer">Terms</a>
    <a href={entry.terms.attribution_url} target="_blank" rel="noreferrer"
      >Attribution</a
    >
    {#each entry.documentation_paths as path}
      <a href={documentationUrl(path)} target="_blank" rel="noreferrer"
        >Source document</a
      >
    {/each}
  </nav>
...
<style>
  ...
  .status-pill.warning {
    border-color: var(--hud-border-solid-soft);
    color: var(--hud-warning);
  }
  .callout {
    border: 1px solid var(--hud-border-solid-soft);
